...
def _resolve_gui_root() -> Path | None:
    env_root = str(os.environ.get("BOCKIS_GUI_ROOT", "")).strip()
    if env_root:
        try:
    ...
    # Fallback for embedded usage: walk upwards and detect GUI repo markers.
    for candidate in [APP_DIR, *APP_DIR.parents]:
        if (candidate / "Win_Gui_Module.ps1").exists() and (candidate / "Modules" / "Core").exists():
            return candidate

    return None


DASHBOARD_ROOT = APP_DIR
GUI_ROOT = _resolve_gui_root()
REPO_ROOT = GUI_ROOT if GUI_ROOT else DASHBOARD_ROOT
LOG_DIR = DASHBOARD_ROOT / "Data" / "Logs"
DASHBOARD_REQUIREMENTS = APP_DIR / "requirements.txt"
CUSTOM_LAUNCHERS_FILE = DASHBOARD_ROOT / "Data" / "dashboard_launchers.json"
LEGACY_CUSTOM_LAUNCHERS_FILE = (GUI_ROOT / "Data" / "dashboard_launchers.json") if GUI_ROOT else None
...
def _load_custom_launchers() -> list[dict]:
    source_file = CUSTOM_LAUNCHERS_FILE
    if not source_file.exists() and LEGACY_CUSTOM_LAUNCHERS_FILE and LEGACY_CUSTOM_LAUNCHERS_FILE.exists():
        source_file = LEGACY_CUSTOM_LAUNCHERS_FILE
...
def api_capabilities() -> dict:
    dependency_checker_available = bool((REPO_ROOT / "Modules" / "Core" / "DependencyChecker.psm1").exists())
    lhm_dll_available = bool((REPO_ROOT / "Lib" / "LibreHardwareMonitorLib.dll").exists())

    return {
        "ok": True,
        "service": "python-dashboard",
        "version": "4.2.2",
        "required_gui_version": ">=4.2.0",
        "dashboard_root": str(DASHBOARD_ROOT),
        "gui_root": str(GUI_ROOT) if GUI_ROOT else None,
        "log_dir": str(LOG_DIR),
        "features": {
            "audio": bool(_ensure_audio_backend()),
            "dependency_checker": dependency_checker_available,
            "lhm_dll": lhm_dll_available,
            "custom_launchers": True,
            "git_context": bool((REPO_ROOT / ".git").exists()),
        },
    }
...
def api_restart() -> dict:
    ...
    script_path = (GUI_ROOT / "Win_Gui_Module.ps1") if GUI_ROOT else Path()
    ...
    return {"success": True, "message": "Dashboard laeuft standalone; GUI-Neustart nicht verfuegbar."}
